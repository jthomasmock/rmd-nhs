...
meta() %>%
  ...
  meta_social(
    ...
    url = "https://jthomasmock.github.io/rmd-nhs/index.html",
    image = "https://images.unsplash.com/photo-1436918898788-ebce04d38e46?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1471&q=80",
    og_type = "website",
    og_author = "Tom Mock",
...
<div class="my-footer"><span>colorado.rstudio.com/rsc/cs-prez/</span></div>
...
.pull-left[
`{r, dpi = 72, fig.dim = c(5,3)}`

```{r surv-plot, eval = TRUE, dpi=72, echo=FALSE, fig.dim = c(5,3)}
```
...
.pull-left[
`{r, dpi = 300, fig.dim = c(5,3)}`

```{r surv-plot, eval = TRUE, dpi=300,echo=FALSE, fig.dim = c(5,3)}
```
...
.pull-right[
`{r, fig.retina=3, fig.dim = c(5,3)}`

```{r surv-plot, eval = TRUE, fig.retina=3,echo=FALSE, fig.dim = c(5,3)}
```
...
surv_days_plot + age_histogram + plot_annotation(tag_levels = "A")
```
...
```{r, fig.dim=c(8,4), fig.retina=1}
# using fig.dim=c(8,4), fig.retina=1
surv_days_plot + age_histogram + plot_annotation(tag_levels = "1")
```
